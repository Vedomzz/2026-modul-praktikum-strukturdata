...
> 🔧 **Koreksi Feedback (13 Jun 2026):** M11, M12 sebelumnya ter-nilai 0/parsial karena file tidak terdeteksi (variasi nama folder / push baru). Setelah pull & evaluasi ulang satu per satu → semua **PASSED**. M12 T1 `SyntaxError` (stray `"""` di akhir file) → T1=0, M12=67. M11 lengkap & PASSED → 100. Rata-rata 74.38→87.23.
...
- **Pengecekan Kode:** File `tugas1` hanya berisi 1 baris kosong — tidak ada implementasi kode apapun.
- **Hasil Testing:** **TIDAK ADA KODE** ❌ (0%)

### 2. Tugas 2: Implementasi Deque + Palindrome
- **Pengecekan Kode:** `Deque` dan `is_palindrome()` diimplementasikan dengan benar. Semua 7 test lulus.
- **Hasil Testing:** **PASSED** ✅ (100%)
...
**NILAI MODUL 6: 67** *(T1:0 + T2:100 + T3:100) / 3 ≈ 67*
...
- **Pengecekan:** PDF `Tugas7_10251074_Muhammad Rizqi Akbar.pdf` ada di folder modul-07.
- **Hasil:** **DIKUMPULKAN** ✅ (100%)

### 2. Tugas 2: Sistem Manajemen Perpustakaan
- **Pengecekan Kode:** Implementasi menggunakan 4 struktur data (LinkedList, Stack, Queue, PriorityQueue). Semua test lulus.
...
- **Pengecekan Kode:** Berhasil menghitung height skewed tree n=10000 tanpa RecursionError. Tabel timing dengan kolom perbandingan tercetak lengkap.
- **Hasil Testing Terminal:** **PASSED** ✅ (100%)
